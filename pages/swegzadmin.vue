  ...
  export default {
    ...
    addRate(){
      if (this.name&&this.max&&this.min&&this.rate) {
        let newRate={name : this.name,value :this.rate,minQuantity :this.min, maxQuantity:this.max}
      
       this.$fireStore
        .collection("rates")
        .add(newRate)
        .then((e) => {alert(" rate sent" + e.id); this.refresh()})
        .catch((e) => console.log(e));
      }else{alert("ensure all fields ar filled")}
    },
    deleteRate(doc) {
      this.$fireStore
        .collection("rates")
